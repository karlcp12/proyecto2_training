...
export const obtenerSolicitudes = async (req, res) => {
    const query = `
        SELECT s.ID_SOLICITUD as id_solicitud, 
               s.ID_APRENDIZ as id_aprendiz, CONCAT(a.NOMBRE, ' ', a.APELLIDO) as nombre_aprendiz,
...
export const actualizarSolicitud = async (req, res) => {
    ...
    try {
        const [result] = await pool.execute(query, [id_aprendiz, codigo_material, id_ficha, cantidad, cleanFecha, estado, id]);
...
// Obtener solicitudes por aprendiz (anteriormente por usuario)
export const obtenerSolicitudesPorUsuario = async (req, res) => {
    const { id_usuario } = req.params; // Se recibe como id_usuario por compatibilidad con la ruta
    const query = `
